...
async function main(): Promise<void> {
  ...
  // Upstream classification: marker-based, with stale-marker lazy-unlink and
  // NON-destructive reads (markers are no longer consumed on classify — this
  // also subsumes the COR-012 .user-stop concern, since .user-stop is never
  // unlinked here unless it is past MARKER_TTL_MS).
  const classified = classifyFromMarkers(stateDir, markers);
  let endType = classified.endType;
  let reason = classified.reason;

  // Compaction detection (local): if no fresh marker classified this exit
  // (endType still 'crash'), check whether the parent process (Claude Code) is
  // still alive. An alive parent + no marker = auto-compaction firing SessionEnd
  // without a real exit — reclassify so it does not surface as a crash.
  if (endType === 'crash') {
    try {
