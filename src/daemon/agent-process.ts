...
export class AgentProcess {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      const startedPid = this.pty.getPid();
      if (startedPid !== null) {
        this.persistPid(startedPid);
      }
  ...
  private buildContinuePrompt(): string {
    ...
    // B-preference (feedback_compaction_notifications): a --continue refresh keeps full
    // conversation history, so the agent should NOT re-read bootstrap files and must NOT
    // send a back-online Telegram (compaction/restart is disk-only, not a user-facing ping).
